...
class Heatmap555ProcessorMixin:
    ...
    def _get_r555_global_noise_threshold(self, settings):
        if 'global_noise_threshold' in settings:
            return max(0.0, float(settings.get('global_noise_threshold', R_HEATMAP_DELTA_THRESHOLD)))
        values = settings.get('global_channel_thresholds', [])
        if isinstance(values, (list, tuple, np.ndarray)) and len(values) > 0:
            return max(0.0, float(np.mean(np.asarray(values, dtype=np.float64))))
        return max(0.0, float(settings.get('delta_threshold', R_HEATMAP_DELTA_THRESHOLD)))
