 ...
 *     - initIADC(): scan FIFO data-valid level changed Dvl1 -> Dvl2 (only
 *       affects interrupt/DMA-driven paths; no effect on this sketch's
 *       polling-based FIFO drain).
 *     - initIADC(): FIFO flush now uses IADC_pullScanFifoData() instead of
 *       IADC_pullScanFifoResult() (raw register read, no alignment/struct
 *       conversion -> slightly cheaper; functionally equivalent for a
 *       discard-only flush).
 *     - CMD_RUN: re-init guard widened from `g_configDirty` to
...
static const int PIN_DRDY     = D7;   // Active-HIGH data-ready output to Teensy pin 0
...
static const uint32_t MUX_SETTLE_US      = 3; // <<========= Optimal for ghosting is 20us. Minimal is 3
...
static inline void drdyWrite(bool asserted) {
  digitalWrite(PIN_DRDY, asserted ? HIGH : LOW);
}
...
static void initIADC()
{
  ...
  const PinName n1 = pinToPinName(PIN_ADC_MUX1);
  const PinName n2 = pinToPinName(PIN_ADC_MUX2);
  ...
  constexpr IADC_CfgAdcMode_t adcMode = iadcCfgModeNormal;

  IADC_Init_t       init       = IADC_INIT_DEFAULT;
  IADC_AllConfigs_t allConfigs = IADC_ALLCONFIGS_DEFAULT;

  // Best choice for repeated, low-latency acquisition.
  init.warmup = iadcWarmupKeepWarm; // should be iadcWarmupKeepWarm for fast continuous conversion (was iadcWarmupNormal)

  init.srcClkPrescale =
      IADC_calcSrcClkPrescale(IADC0, IADC_SRC_CLK_HZ, 0);

  IADC_Config_t &cfg = allConfigs.configs[0];

  cfg.adcMode     = adcMode;
  cfg.reference   = g_vref_sel;
  cfg.vRef        = g_vref_mV;
  cfg.osrHighSpeed = g_osr;
  cfg.analogGain  = g_gain;
  cfg.digAvg      = iadcDigitalAverage1;
  ...
  initScan.showId       = false;
  initScan.triggerSelect = iadcTriggerSelImmediate;
  initScan.triggerAction = iadcTriggerActionOnce;
  initScan.start         = false;

  // This is mainly relevant for interrupt or DMA operation.
  // It does not make FIFO-count polling faster.
...
static void prefetchNextBlockIfNeeded() {
  if (!isRunning || !streamRespArmed || nextBlockReady || gState != RESP_ARMED) {
    return;
  }

  // Once a streaming response is armed, use the alternate buffer to keep
  // capturing ahead whenever possible. If the just-finished transfer carried
  // CMD_STOP in spiRxSink[0], skip prefetch so the next response can become
  // the final ACK instead of another data block.
  if (xferDone && spiRxSink[0] == CMD_STOP) {
    return;
  }
...
static uint32_t processCommand(const uint8_t *frame) {
  ...
  switch (cmd) {
    ...
    case CMD_RUN: {
      ...
      if (g_configDirty || !g_iadcReady) { // changed 7.21.26: This ensures initialization occurs even if the peripheral became unavailable
        initIADC();
        buildEntryList();
      }
...
void setup() {
  ...
  // Active-HIGH DRDY output to Teensy
  ...
  armCmd();
  Serial.println(F("MG24 Dual-MUX Slave ready (DRDY on D7)"));
}
