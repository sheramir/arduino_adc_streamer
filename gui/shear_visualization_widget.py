...
class ShearArrowRenderMixin:
    """Shared shear-arrow geometry and rendering.

    Hosts must provide ``arrow_line_item``, ``arrow_head_item``, ``arrow_color``,
    ``arrow_base_width_px``, ``arrow_width_scales`` and ``last_arrow_geometry``.
    Each widget supplies its own ``calculate_arrow_geometry``, which is the only
...
class ShearVisualizationWidget(ShearArrowRenderMixin, QWidget):
