...
class PiezoHeatmapProcessorMixin:
    ...
    def _get_heatmap_position_for_display_spec(self, spec, spec_index, channel_to_sensor):
        ...
        if isinstance(key, tuple) and len(key) >= 3 and key[0] == 'sensor':
            ...
            if candidate in self._threshold_label_order():
                return candidate

        label = str(spec.get('label', '') if isinstance(spec, dict) else '').strip().upper()
        for separator in ('_', '-'):
            if separator in label:
                candidate = label.rsplit(separator, 1)[-1]
                if candidate in self._threshold_label_order():
                    return candidate

        if spec_index < len(channel_to_sensor):
            candidate = str(channel_to_sensor[spec_index]).strip().upper()
            if candidate in self._threshold_label_order():
                return candidate

        return None

    def _get_heatmap_package_id_for_display_spec(self, spec, spec_index):
        key = spec.get('key') if isinstance(spec, dict) else None
        ...
        package_index = int(spec_index) // max(1, len(self._threshold_label_order()))
    ...
    def _get_heatmap_baseline_key_for_display_spec(self, spec):
        ...
        if isinstance(key, tuple):
            if key and key[0] == 'sensor' and len(key) >= 4:
                return key[3]
            if key and key[0] == 'mux' and len(key) >= 3:
                return key[2]
            if key and key[0] == 'adc' and len(key) >= 2:
                return key[1]
        return None
    ...
    def _get_piezo_global_noise_threshold(self, settings):
        if 'global_noise_threshold' in settings:
            return float(settings.get('global_noise_threshold', 0.0))
        if 'global_channel_thresholds' in settings:
            values = settings.get('global_channel_thresholds', [])
            if isinstance(values, (list, tuple, np.ndarray)) and len(values) > 0:
                return float(np.mean(np.asarray(values, dtype=np.float64)))
        return float(settings.get('general_threshold', settings.get('magnitude_threshold', 0.0)))

    def _calibrate_heatmap_sensor_values(self, sensor_values, settings, sensor_id):
        ...
        if isinstance(sensor_calibration_dict, dict):
            calib_data = sensor_calibration_dict.get(str(sensor_id), {})
            if not calib_data:
                calib_data = sensor_calibration_dict.get(str(sensor_id).upper(), {})
            if isinstance(calib_data, dict):
                per_sensor_gains = self._build_piezo_channel_value_array(
                    calib_data.get('gains', []),
                    1.0,
                    len(sensor_values),
                )
                per_sensor_thresholds = self._build_piezo_channel_value_array(
        ...
        calibrated = []
        for value, noise, gain, per_gain in zip(sensor_values, noise_floor, calibration, per_sensor_gains):
            adjusted = max(0.0, float(value) - float(noise))
            calibrated.append(adjusted * float(gain) * float(per_gain))

        global_noise_threshold = self._get_piezo_global_noise_threshold(settings)
        thresholded = []
        for value, per_thresh in zip(calibrated, per_sensor_thresholds):
            threshold = global_noise_threshold + float(per_thresh)
            thresholded.append(value if value >= threshold else 0.0)
        return thresholded
    ...
    def _compute_channel_intensities_from_display_specs(self, settings, data_array, timestamps, avg_sample_time_us):
        ...
        for spec_index, spec in enumerate(display_specs):
            if not isinstance(spec, dict):
                continue
            position = self._get_heatmap_position_for_display_spec(spec, spec_index, channel_to_sensor)
            if position not in sensor_labels:
                continue

            sample_indices = [
                int(sample_index)
                for sample_index in spec.get('sample_indices', [])
                if 0 <= int(sample_index) < data_array.shape[1]
            ]
            if not sample_indices:
                continue

            package_id = self._get_heatmap_package_id_for_display_spec(spec, spec_index)
            if package_id not in package_samples:
                package_samples[package_id] = {label: [] for label in sensor_labels}
                package_order.append(package_id)

            spec_data = data_array[:, np.asarray(sample_indices, dtype=np.int32)].reshape(-1)
            baseline_key = self._get_heatmap_baseline_key_for_display_spec(spec)
            if baseline_key in channel_to_baseline:
                spec_data = spec_data - float(channel_to_baseline[baseline_key])

            package_samples[package_id][position].append(spec_data)

        if not package_order:
            return None

        sample_rate_hz = 1000000.0 / avg_sample_time_us
        samples_per_repeat = max(
            1.0,
        ...
        for package_index, package_id in enumerate(package_order[:MAX_SENSOR_PACKAGES]):
            ...
            for position in sensor_labels:
                ...
                if samples_for_position:
                    channel_samples.append(np.concatenate(samples_for_position))
                else:
            ...
            sensor_values = [
                float(rms_values[index]) if index < len(rms_values) else 0.0
                for index in range(len(sensor_labels))
            ]
            thresholded = self._calibrate_heatmap_sensor_values(sensor_values, settings, package_id)
    ...
    def compute_channel_intensities(self, settings):
        ...
        spec_values = self._compute_channel_intensities_from_display_specs(
            settings,
            data_array,
            timestamps,
            avg_sample_time_us,
        ...
        if not sensor_package_groups:
            return None
        sensor_package_groups = list(sensor_package_groups)[:MAX_SENSOR_PACKAGES]
        ...
        for package_index, group in enumerate(sensor_package_groups):
            ...
            thresholded = self._calibrate_heatmap_sensor_values(sensor_values, settings, sensor_id)
