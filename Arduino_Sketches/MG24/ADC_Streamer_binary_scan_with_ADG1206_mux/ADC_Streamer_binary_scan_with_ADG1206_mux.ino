...
// ---------------------------------------------------------------------
// External MUX configuration (ADG1206 16:1)
// ---------------------------------------------------------------------
// IMPORTANT: In this combined sketch, the "channels" command refers to
// MUX channel numbers (0..15), NOT MG24 analog pins.
//
// Wiring (defaults match ADC_MUX1_test.ino):
//   - ADG1206 A0..A3  -> MG24 digital pins below
//   - ADG1206 EN      -> MG24 digital pin below
...
uint8_t  channelSequence[MAX_SEQUENCE_LEN]; // user-chosen MUX channels in sweep
...
int   groundPin              = 0;   // MUX channel used for ground dummy reads
...
uint8_t entryMuxChannel[MAX_SCAN_ENTRIES]; // MUX channel for each scan entry (ground or data)
...
static IADC_PosInput_t  g_muxPosInput = iadcPosInputPortAPin0; // overwritten in init
...
static inline uint16_t iadcReadOnce12() {
  ...
  return (uint16_t)(res.data & 0x0FFF);
}
...
static void initIADC_ScanMultiChannel() {
  ...
  // ADC pin (MUX COM) setup
  pinMode(PIN_ADC_MUX_COM, INPUT);
  PinName adcName = pinToPinName(PIN_ADC_MUX_COM);
  if (adcName == PIN_NAME_NC) {
    Serial.println(F("# ERROR: PIN_ADC_MUX_COM is not a valid MCU pin. Check wiring/pin number."));
    isRunning     = false;
    timedRun      = false;
    g_iadcReady   = false;
    g_configDirty = false;
  ...
  // Enable clocks (GPIO needed for analog bus allocation)
  CMU_ClockEnable(cmuClock_GPIO, true);
  CMU_ClockEnable(cmuClock_IADC0, true);

  // Map GPIO pin -> IADC pos input
  uint32_t pinIndex = (uint32_t)adcName - (uint32_t)PIN_NAME_MIN;
  g_muxPosInput = GPIO_to_ADC_pin_map_local[pinIndex];
  allocateAnalogBusForPin(adcName);

  // Build the per-sweep entry list: entryMuxChannel[] + isGroundEntry[]
  ...
  for (uint8_t i = 0; i < channelCount && entryIndex < MAX_SCAN_ENTRIES; ++i) {
    ...
    if (haveGround && isNew && entryIndex < MAX_SCAN_ENTRIES) {
      entryMuxChannel[entryIndex] = (uint8_t)groundPin; // interpreted as MUX channel
      isGroundEntry[entryIndex]   = true;
      entryIndex++;
    }

    for (uint16_t r = 0; r < repeatCount && entryIndex < MAX_SCAN_ENTRIES; ++r) {
      entryMuxChannel[entryIndex] = chan;
  ...
  if (entryIndex == 0) {
    Serial.println(F("# ERROR: no valid scan entries configured. Stopping run."));
    scanEntriesPerSweep = 0;
    samplesPerSweep     = 0;
    isRunning           = false;
    timedRun            = false;
  ...
  init.srcClkPrescale = IADC_calcSrcClkPrescale(IADC0, IADC_SRC_CLK_TARGET_HZ, 0);
  ...
  singleInput.posInput = g_muxPosInput;
  singleInput.negInput = iadcNegInputGnd;
  singleInput.configId = 0;
...
void discardWarmupSweeps(uint16_t warmupSweeps) {
  ...
  if (!g_iadcReady) return;

  for (uint16_t s = 0; s < warmupSweeps; ++s) {
    for (uint16_t e = 0; e < scanEntriesPerSweep; ++e) {
      muxSelect(entryMuxChannel[e]);
      (void)iadcReadOnce12();
      if (!isRunning && !timedRun) {
        // stop requested during warmup
        return;
      }
    }
  }
...
void doOneBlock() {
  ...
  if (!g_iadcReady) {
    Serial.println(F("# ERROR: IADC not ready."));
    isRunning = false;
    timedRun  = false;
    return;
  }
...
bool handleChannels(const String &args) {
  ...
  while (i < len && channelCount < MAX_SEQUENCE_LEN) {
    ...
    int val = token.toInt();
    if (val < 0 || val > (int)MUX_CHANNEL_MAX) {
      Serial.print(F("# ERROR: channel out of range (0-"));
      Serial.print(MUX_CHANNEL_MAX);
      Serial.println(F(")"));
      continue;
    }
...
bool handleGround(const String &args) {
  ...
  } else { // command specified a MUX channel for ground dummy reads
    int pin = a.toInt();
    if (pin < 0 || pin > (int)MUX_CHANNEL_MAX) {
      Serial.print(F("# ERROR: ground channel out of range (0-"));
      Serial.print(MUX_CHANNEL_MAX);
      Serial.println(F(")"));
      return false;
    }
...
void printStatus() {
  ...
  Serial.println(F("# ---- external mux ----"));
  Serial.print(F("# mux_com_adc_pin: "));
  Serial.println(PIN_ADC_MUX_COM);
  Serial.print(F("# mux_settle_us: "));
  Serial.println(g_mux_settle_us);
  Serial.print(F("# mux_channel_max: "));
  Serial.println(MUX_CHANNEL_MAX);
