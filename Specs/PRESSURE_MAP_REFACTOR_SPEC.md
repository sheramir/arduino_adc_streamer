...
Peak-height extrapolation is controlled only by `peak_gain_slope_per_mm`: the inferred gain is `min(maximum_peak_gain, 1 + distance_mm * peak_gain_slope_per_mm)`. Spatial reach is controlled separately by `natural_decay_reference_distance_mm`, `decay_amplitude_reference`, and minimum/maximum decay reach. The default amplitude reference of 1.0 is a named integrated-signal-domain starting point and must be calibrated against representative capture data.
